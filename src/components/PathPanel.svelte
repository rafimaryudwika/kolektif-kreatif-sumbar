...
<script lang="ts">
  ...
  import { edgePhrase, hopLabel } from '../lib/relationships';
...
<div class="mt-4">
  ...
  {:else if searched && !result}
    <p class="text-sm text-ink-muted">
      No route between {nameOf(fromId)} and {nameOf(toId)} within four hops. They may
      still be connected further out — the query stops there to keep the answer fast.
    </p>
  {:else if result}
    <p class="text-sm text-ink-muted">
        ...
        <li>
          ...
          {#if result.pathTypes[index]}
            <p class="ml-3.5 border-l border-border py-1 pl-4 text-xs text-ink-subtle">
              {edgePhrase(result.pathTypes[index])}
            </p>
          {/if}
        </li>
      {/each}
